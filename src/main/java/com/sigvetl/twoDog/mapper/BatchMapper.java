...
//            "VALUES(#{name}, #{type}, #{ibu}, #{recipeLink}, #{userId})")
    @Insert("INSERT INTO BATCH(VOLUME, OG, FG, ABV, USERID) " +
            "VALUES(#{volume}, #{og}, #{fg}, #{abv}, #{userId})")
...
//            "RECIPELINK = #{recipeLink}, USERID = #{userId} WHERE BATCHID = #{batchId}")
    @Update("UPDATE BATCH SET VOLUME = #{volume}, OG = #{og}, FG = #{fg}, ABV = #{abv}, USERID = #{userId} WHERE BATCHID = #{batchId}")
